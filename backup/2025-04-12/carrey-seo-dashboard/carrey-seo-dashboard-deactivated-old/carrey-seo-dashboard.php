 ...
 * Description: An advanced SEO dashboard for WordPress
 * Version: 1.0.0
 * Author: Carrey
...
define('CARREY_SEO_PATH', plugin_dir_path(__FILE__));
...
function carrey_seo_dashboard_init() {
    try {
        ...
        error_log('Carrey SEO Dashboard Error: ' . $e->getMessage());
        add_action('admin_notices', function() use ($e) {
            echo '<div class="error"><p>Carrey SEO Dashboard Error: ' . esc_html($e->getMessage()) . '</p></div>';
        });
    }
}
...
function carrey_seo_dashboard_activate() {
    try {
        ...
        error_log('Carrey SEO Dashboard Activation Error: ' . $e->getMessage());
        wp_die('Carrey SEO Dashboard Activation Error: ' . $e->getMessage());
    }
}
