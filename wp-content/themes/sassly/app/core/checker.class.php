...
if(!class_exists("Sassly_Base")) {
	class Sassly_Base {
    	public $key = "7D064330D57335FA";
    	public $product_id = "4";
    	public $product_base = "sassly";
    	public $server_host = "https://themecrowdy.com/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->has_check_update) {
				if(function_exists("add_action")){
					add_action( 'admin_post_sassly_fupc', function(){
					...
					if($this->is_theme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if (  plugin_basename( $this->plugin_file ) == $plugin_file ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=sassly_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		 ...
		 public function theme_force_update(){
            ...
            echo wp_kses_post('<h1>'.__("Update Checking..",'sassly').'</h1>');
		...
		final function _remove_wp_plugin_license(&$message=''){
			...
			$obj=self::get_instance(__FILE__);
			$obj->set_email_address(get_option('sassly_lic_email'));
			if(!empty($old_respons->is_valid) || 1==1) {
