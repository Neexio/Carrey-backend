...
class FW_Extension_Backups_Demo extends FW_Extension {
	...
	public function _admin_action_admin_menu() {
		if (
			...
			if(!apply_filters('wcf_sandbox_backups_demo_enable_menu', false)){
				return;
			}
			return;
		}
	...
	public function _display_page() {
		...
		if (function_exists('is_wpe_snapshot') && !is_wpe_snapshot()) {		   
			...
			sprintf(
				esc_html__('Please use open click here to %s.', 'fw'),
				'<a href="'. esc_attr(admin_url('admin.php?page=wcf-theme-demo-import')) .'">'
				. esc_html__('Resolve', 'fw')
				. '</a>'
			),
			'</p>
			<script> window.location.href = "'. esc_attr(admin_url('admin.php?page=wcf-theme-demo-import')) .'"; </script>
			</div>';
	...
	private function is_page() {
		...
		if(property_exists($current_screen, 'id') && 'sassly-theme_page_fw-backups-demo-content' === $current_screen->id){
	...
	public function _action_admin_enqueue_scripts() {
		if ($this->is_page()) {
			...
			wp_localize_script(
				...
				array(
					'backup_link' => admin_url('admin.php?page=wcf-theme-demo-import'),	
					'ajax_action' => array(
	...
	public function do_install(FW_Ext_Backups_Demo $demo) {
		...
		$collection->add_task(new FW_Ext_Backups_Task(
			...
			array(
				...
				// used only for  -Backups-Extension/issues/15
				'demo_id' => $demo->get_id(),
			)
		));
